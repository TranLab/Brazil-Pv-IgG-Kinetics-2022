---
title: "*P. vivax* antibody Data Visualization and Analysis for Manuscript"
author: "Tuan M. Tran"
date: "07/18/2022"
...
tab_dat <- alldat %>%
  dplyr::select(Subject, city, gender, age, "Years of Amanzonia State") %>%
  distinct() %>%
...
myVars <- c("Gender", "Age, years", "Years residence in Amazon basin")
catVars <- c("Gender")
nonnormalVars <- c("Age, years", "Years residence in Amazon basin")
...
```{r glmm adaptibe mixed model, echo = TRUE, eval= TRUE, warning=FALSE, message=FALSE}
...
```{r make table glmmTMB, echo = TRUE, eval = TRUE, warning=FALSE, message=FALSE, fig.width=12}
library(tableone)
library(flextable)
library(officer)
source("customtab.R")
...
```{r render table glmmTMB, echo = FALSE, warning=FALSE, message=FALSE, fig.width=12}
flextable_glmmTMB_res
```

# Longley dataset

## Compare with published Longley et al. PLoS NTD 2017 dataset207
...
## Plot Forest Plot comparing Tenshi et al vs Longley et al
