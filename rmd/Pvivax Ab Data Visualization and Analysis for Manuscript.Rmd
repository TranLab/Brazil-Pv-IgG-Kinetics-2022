...
```{r local directories, echo=TRUE, message=FALSE, warning=FALSE}
...
```{r participant characteristics, echo = TRUE, warning=FALSE, message=FALSE}
...
```{r plot correlation heatmap, echo = TRUE, warning=FALSE, message=FALSE, fig.width=8, fig.height= 8, fig.align='center'}
...
```{r plot all sero sites, fig.align='center', fig.height=8, fig.width=8}
all_sero 
```
...
```{r seroprevalence by ag by  site, echo = TRUE, warning=FALSE, message=FALSE, fig.align='center', fig.width=12, fig.height=12}
...
```{r plot seropositivity by study site, fig.align='center', fig.width= 10, fig.height=10}
all_sero_plots
```
...
```{r save seroprevalence plot, echo=FALSE, eval=FALSE}
figdir <- "/Users/tuantran/OneDrive - Indiana University/FromGoogle/Manuscripts/Brazil Antibody Kinetics Manuscript/PLOS NTD Submission August 2022/PLoS NTD Revison/"
ggsave(plot = seropos_ag_time,
       filename = paste0(figdir, "Fig 2 Seroprevalence by Ag Over Time Continuous 10172022 REV.pdf"),
       units = "in",
       width = 7.5,
       height = 7.5)
```
...
```{r estimate as exponential decay facet by antigen, echo = FALSE, eval=FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.width=9, fig.height=9}
...
```{r make table S2, echo=FALSE, eval=FALSE}
...
```{r compare longley published, echo=TRUE}
...
```{r plot forest plot, echo=TRUE, fig.align='center', fig.width=8, fig.height=4}
CI_plots
```
